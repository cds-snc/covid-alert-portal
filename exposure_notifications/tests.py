...
class SearchView(NotificationsTestCase):
    ...
    def test_post_request(self):
        ...
        self.login()
        response = self.client.post(reverse("exposure_notifications:start"), {'search_text': 'nandos'})
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse('exposure_notifications:start') + '?search_text=nandos')

    def test_search_functionality(self):
        # Test that GET with a query param produces search results
        self.login()
        response = self.client.get(reverse("exposure_notifications:start"), {'search_text': 'nandos'})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'search.html')
    ...
    def test_province_filter(self):
        ...
        self.login()
        response = self.client.get(reverse("exposure_notifications:start"), {'search_text': 'bobs'})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'search.html')
...
class ProfileView(NotificationsTestCase):
    ...
    def test_profile_view(self):
        ...
        response = self.client.get(reverse("exposure_notifications:profile", args=[location.id]))
        self.assertEqual(response.status_code, 200)
...
class SeverityView(NotificationsTestCase):
    ...
    def test_severity_view(self):
        ...
        location = Location.objects.get(name='Nandos')
        self.client.get(reverse("exposure_notifications:profile", args=[location.id]))
        self.client.post(reverse("exposure_notifications:datetime"), {'day': 1, 'month': 1, 'year': 2021})
        response = self.client.get(reverse("exposure_notifications:severity"))
        self.assertEqual(response.status_code, 200)
...
class HistoryView(NotificationsTestCase):
    ...
    def test_post_request(self):
        # Test that posting the form will redirect back to the same page, but with query params
        self.login()
        response = self.client.post(reverse("exposure_notifications:history"), {'search_text': 'nandos'})
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse('exposure_notifications:history') + '?search_text=nandos')

    def test_search_functionality(self):
        # Test that GET with a query param produces search results
        self.login()
        response = self.client.get(reverse("exposure_notifications:history"), {'search_text': 'nandos'})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'history.html')
...
class DetailsView(NotificationsTestCase):
    ...
    def test_details_view(self):
        ...
        response = self.client.get(reverse("exposure_notifications:details", args=[notification.id]))
        self.assertEqual(response.status_code, 200)
