...
class LoggerBackend:
    ...
    def crud(self, crud_info):
        ...
        if model == "profiles.healthcareuser" and (event_type == "CREATE" or event_type == "DELETE"):

            portal_users =  HealthcareProvince.objects.annotate(num_admins=Count('healthcareuser', filter=Q(healthcareuser__is_admin="True")),
            num_super_admins = Count('healthcareuser', filter=Q(healthcareuser__is_superuser="True")),
            num_staff=Count('healthcareuser', filter=Q(healthcareuser__is_admin="False"))).values_list("name","num_admins", "num_super_admins", "num_staff")
